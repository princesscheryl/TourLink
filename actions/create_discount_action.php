...
try {
    ...
    // Create discount
    $result = create_discount_code_ctr(
        $code,
        $discount_type,
        $discount_value,
        $min_booking_amount,
    ...
    if ($result) {
        ...
        throw new Exception('Failed to create discount code. Code may already exist.');
    }

} catch (Exception $e) {
    error_log("Discount creation error: " . $e->getMessage());
